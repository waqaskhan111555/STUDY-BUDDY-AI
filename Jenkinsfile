pipeline {
    agent any
    environment {
        DOCKER_HUB_REPO = "waqaskhan111555/studybuddy"
    ...
    stages {
        ...
        stage('Update Deployment YAML with New Tag') {
            steps {
                script {
                    sh """
                    sed -i 's|image: waqaskhan111555/studybuddy:.*|image: waqaskhan111555/studybuddy:${IMAGE_TAG}|' manifests/deployment.yaml
                    """
                }
            }
        }
            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'github-token', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                        sh '''
                        git config user.name "waqaskhan111555"
                        git config user.email "waqaskhan111555111555@gmail.com"
                        git add manifests/deployment.yaml
                        git commit -m "Update image tag to ${IMAGE_TAG}" || echo "No changes to commit"
                        git push https://${GIT_USER}:${GIT_PASS}@github.com/waqaskhan111555/STUDY-BUDDY-AI.git HEAD:main
                        '''
                    }
                }
            }
